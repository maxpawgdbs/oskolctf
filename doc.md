...
│   ├── spa.html         # Основное SPA-приложение (Vue.js 3 + Vue Router)
...
│
├── task/                # Файлы для заданий (скачиваемые материалы)
│   └── pwn/
│       └── osint/       # Пример директории для файлов
│
...
1. **Клиентская часть (Vue.js 3 SPA)**:
   - Приложение загружается как единая страница (`spa.html`)
   - Vue Router управляет навигацией между страницами (`/`, `/login`, `/register`, `/board`)
   - Все данные загружаются через REST API (`/api/*`)
   - Интерфейс реактивно обновляется при изменении данных
   - Компоненты: `HomePage`, `LoginPage`, `RegisterPage`, `BoardPage`
   - Встроенная система уведомлений (toasts)
...
2. **Серверная часть (Flask)**:
   ...
   - Отдаёт файлы заданий через `/api/task-file/<task_id>`
...
4. **Конфигурация заданий (tasks.json)**:
   ...
   - Поддерживает поле `file` для скачиваемых материалов
...
**Зависимости:**
- `flask[async]==3.1.1` — веб-фреймворк
- `werkzeug` — утилиты безопасности (устанавливается автоматически с Flask)
...
> **Важно:** Все атрибуты в JSON являются **обязательными**.
...
| `active` | boolean | ✅ | Статус задания (`true` — активно, `false` — скрыто) |
| `file` | string | ❌ | Относительный путь к файлу для скачивания. Указывается относительно директории `task/` (например: `osint/test.txt`). Не указывайте `task/` в начале пути |
| `hide_open_button` | boolean | ❌ | Скрывает кнопку "Открыть" в интерфейсе задания |
...
| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/` | Главная страница (SPA лендинг) |
| GET | `/task0` | Страница задания 0 (флаг на сервере) |
| GET | `/task1` | Страница задания 1 (комментарий в HTML) |
| GET | `/task2` | Страница задания 2 (куки) |
| GET, POST | `/task3` | Страница задания 3 (POST с base64) |
| GET | `/task4` | Страница задания 4 (проверка куки) |
...
| GET | `/api/task-file/<task_id>` | Скачать файл задания |
| GET | `/<path:path>` | Catch-all для SPA роутинга (все не-API маршруты) |

### Frontend (spa.html)

#### Компоненты Vue.js

1. **HomePage** — лендинг с анимацией терминала и частицами
2. **LoginPage** — страница входа (форма с CSRF-защитой)
3. **RegisterPage** — страница регистрации
4. **BoardPage** — основная страница с:
   - Панелью статистики (всего задач, решено, очков)
   - Фильтром по категориям
   - Сеткой карточек задач
   - Таблицей лидеров (топ-50)
   - Модальным окном задачи
...
- **Тёмная тема** — основной фон `#0b0f17`
- **Анимации** — плавные переходы страниц, частицы на фоне, мигающий курсор
- **Toast-уведомления** — зелёные (успех), красные (ошибка), синие (инфо)
- **Карточки задач** — с категориями, сложностью, очками, статусом решения
- **Таблица лидеров** — с подсветкой топ-3 и текущего пользователя

---

### GET `/api/task-file/<task_id>`

Скачать файл задания (требуется авторизация). Файл берётся из директории `task/` согласно полю `file` в `tasks.json`.

**Ответ:**
- Файл для скачивания
- `404` если файл не найден или задание не имеет файла
- `403` если попытка path traversal атаки
...
### Тип 5: Задание с файлом

1. Положите файл в директорию `task/` (например, `task/pwn/challenge.zip`)
2. Добавьте поле `file` в `tasks.json`:

```json
{
    "id": 7,
    "name": "Потерянный файл",
    "category": "Форензика",
    "difficulty": "Среднее",
    "description": "Файл был повреждён. Восстанови его и найди скрытое сообщение.",
    "points": 200,
    "flag": "oskolctf{forensics_lost_file_flag}",
    "url": "/task7",
    "active": true,
    "file": "pwn/challenge.zip"
}
```

3. Игрок сможет скачать файл через кнопку в интерфейсе или по API `/api/task-file/7`
